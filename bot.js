...
💎+v2min   『 لانشاء روم موقت 』

💎+mcskin  『 لرؤية سكنك في مينكرافت 』
...
القسم الثالث مصمم البوت | @-ARMY|XBooMX#3600
...
+support| القسم الثاني  الدعم الفني و المساعدة
قسم الثالث مصمم البوت @ARMY|XBooMX#3600
...
if (message.content === '+support') {
      message.author.send(' |https://discord.gg/WkwG5P6| لـ أي استفسار').catch(e => console.log(e.stack));
...
if (message.content === '+invite') {
      message.author.send(' رابط البوت |  https://discordapp.com/oauth2/authorize?client_id=457619346357878785&permissions=8&scope=bot ').catch(e => console.log(e.stack));
...
if(ReBeL.voiceChannelID !== "ايدي الروم اللي ينقلك لروم ثاني") return console.log("أيرور . ");
ReBeL.guild.createChannel(ReBeL.user.username , 'voice').then((rebeeel) =>{
    rebeeel.setParent("ايدي الكاتاجوري اللي يكون فيها الرومات المؤقته");
...
client.on('message', message => {
 ...
 client.on('message', message => {
    if(message.content === 'xboomx'){
        message.channel.send('n3amm')
    }
});
client.on('message', message => {
    if(message.content === 'yousseftube'){
        message.channel.send('tbi la')
    }
});
	client.on("message", message => {
    var prefix = "+"
